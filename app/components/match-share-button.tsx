...
export function MatchShareButton({ className = "share-zalo-btn", ...match }: Props) {
  ...
  return <div className="match-share-control">
    ...
    {state.fallbackUrl && <input className="match-share-fallback" aria-label="Link lời mời trận đấu" readOnly value={state.fallbackUrl} onFocus={(event) => event.currentTarget.select()} />}
  </div>;
}
