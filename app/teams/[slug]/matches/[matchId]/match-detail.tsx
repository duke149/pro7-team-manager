...
export function MatchDetail({ slug, teamName, userId, detail, canManage, canRespond }: { slug: string; teamName: string; userId: string; detail: MatchDetailModel | null; canManage: boolean; canRespond: boolean }) {
  ...
  async function rsvp(status: AttendanceResponseStatus) { if (!own) return; await mutate(`${base}/attendance`, { action: "respond", status, note: null, expectedUpdatedAt: own.updatedAt }, "POST"); }
  ...
  return <div className="view-stack match-center match-detail-view">
    ...
    <section className="confirmed-card"><div className="confirmed-strip"><CalendarDays size={18} />{match.status === "scheduled" ? "Đã tìm thấy đối thủ" : match.status === "completed" ? "Trận đấu đã hoàn tất" : "Trận đấu đã hủy"}</div><div className="confirmed-body"><div className="card-kicker dark-kicker">{displayDate(match.startsAt).toLocaleUpperCase("vi-VN")}</div><h2>{firstTeam} <em>vs.</em> {secondTeam}</h2><p><MapPin size={17} />{match.venue ?? "Chưa cập nhật địa điểm"}</p>{match.status === "completed" && <strong className="match-detail-score">{teamScore} – {awayScore}</strong>}</div></section>
    {match.status === "scheduled" && canRespond && own && <section className="card rsvp-card"><div className="section-head"><div><span>XÁC NHẬN</span><h2>Bạn có tham gia?</h2></div></div><div className="rsvp-options two-options"><button type="button" disabled={state.pending} className={own.status === "available" ? "active yes" : ""} onClick={() => void rsvp("available")}><Check />Có</button><button type="button" disabled={state.pending} className={own.status === "unavailable" ? "active no" : ""} onClick={() => void rsvp("unavailable")}><X />Không</button></div></section>}
    ...
    <section className="two-col match-detail-grid">
      <article className="analysis-card"><div className="analysis-score"><span>PHÂN TÍCH TRẬN ĐẤU</span><small>{match.status === "completed" ? "KẾT THÚC" : match.status === "cancelled" ? "ĐÃ HỦY" : "SẮP DIỄN RA"}</small><div className="score-board"><b>{firstTeam}</b><strong>{teamScore ?? "—"} <em>–</em> {awayScore ?? "—"}</strong><b className="muted-team">{secondTeam}</b></div></div><div className="analysis-body"><h3>Diễn biến chính</h3>{detail.events.length === 0 ? <p className="match-muted">Chưa có diễn biến được ghi nhận.</p> : detail.events.map((event) => <div className="event-row" key={event.id}><span className={event.teamSide === "opponent" ? "away-event" : ""}>{event.minute}′</span><p>{event.note ?? event.eventType.replaceAll("_", " ")}</p></div>)}{mvp && <div className="motm"><div className="initial-avatar dark-avatar">MVP</div><div><span>CẦU THỦ XUẤT SẮC</span><b>{mvp.displayName ?? "Cầu thủ"}</b><small>{mvp.goals} bàn • {mvp.assists} kiến tạo • {mvp.rating ?? "—"} điểm</small></div><Trophy /></div>}<div className="team-stats">{metricRows(detail.teamMetrics).filter((entry) => entry[1]).map(([label, metric]) => <div className="compare-row" key={label}><b>{metric?.team}</b><span>{label}</span><b>{metric?.opponent}</b></div>)}</div></div></article>
      <article className="card match-attendance-card"><div className="section-head"><div><span>THAM GIA</span><h2>Danh sách xác nhận</h2></div><strong>{match.attendance.available}/{match.attendance.invited}</strong></div>{detail.attendance.length === 0 ? <p className="match-muted">Chưa mời thành viên.</p> : detail.attendance.map((attendance) => <div className="match-attendance-row" key={attendance.userId}><b>{attendance.displayName ?? "Thành viên"}</b><span className={attendance.status}>{attendance.status === "available" ? "Có mặt" : attendance.status === "unavailable" ? "Vắng" : "Đang chờ"}</span></div>)}</article>
    </section>
