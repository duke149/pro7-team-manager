...
function serverMessage(value: unknown, fallback: string) {
...
): readonly (readonly [string, TeamMetric])[] {
  if (!metrics) return [];
  return [
    ["Kiểm soát", metrics.possession],
    ["Cú sút", metrics.shots],
    ["Trúng đích", metrics.shotsOnTarget],
    ["Phạt góc", metrics.corners],
  ].filter((entry): entry is [string, TeamMetric] => entry[1] !== undefined);
}
...
export function MatchDetail({
  ...
  const [selected, setSelected] = useState(
    () =>
      new Set(
        detail?.inviteCandidates.map((candidate) => candidate.userId) ?? [],
      ),
  );
  const rsvpClosed = useRsvpDeadlineClosed(
    detail?.match.rsvpDeadline ?? null,
    now,
  );
  if (!detail)
    return (
      <div className="view-stack match-center" data-state="error">
        <section className="card match-state">
  ...
  const { match } = detail;
  const own =
    detail.attendance.find((attendance) => attendance.userId === userId) ??
    null;
  ...
  async function mutate(
    ...
    try {
      ...
      if (!response.ok) {
        setState({
          pending: false,
          message: serverMessage(body, "Không thể cập nhật trận đấu."),
          success: false,
        });
        return false;
      }
      ...
      setState({
        pending: false,
        message: "Không thể cập nhật trận đấu.",
        success: false,
      });
      return false;
    }
  ...
  async function invite() {
    await mutate(
      `${base}/attendance`,
      { action: "invite", userIds: [...selected] },
      "POST",
    );
  }
  ...
  return (
    <div className="view-stack match-center match-detail-view">
      <a
        className="text-button squad-back-link"
        href={`/teams/${encodeURIComponent(slug)}/matches`}
      >
        <ArrowLeft size={16} /> Quay lại trận đấu
      </a>
      <section className="confirmed-card ticket-style">
        ...
        <div className="confirmed-body">
          ...
          <p>
            <MapPin size={17} />
            {match.venue ?? "Chưa cập nhật địa điểm"}
          </p>
          {match.status === "completed" && (
            <strong className="match-detail-score">
              {teamScore} – {awayScore}
            </strong>
          )}
          <div
          ...
          >
            <a
              ...
              href={`/teams/${encodeURIComponent(slug)}/tactics/${encodeURIComponent(match.id)}`}
            >
      ...
      {match.status === "scheduled" && canRespond && own && (
        <section className="card rsvp-card">
          ...
          <div className="rsvp-options two-options">
            ...
            <button
              type="button"
              disabled={state.pending || rsvpClosed}
              className={own.status === "unavailable" ? "active no" : ""}
              onClick={() => void rsvp("unavailable")}
            >
              <X />
              Không
            </button>
          </div>
          {rsvpClosed && <p className="match-muted">Đã hết hạn xác nhận.</p>}
        </section>
      )}
      {state.message && (
        <p
          className={`match-message ${state.success ? "success" : "error"}`}
      ...
      <section className="two-col match-detail-grid">
        <article className="analysis-card">
          <div className="analysis-score">
            ...
            <div className="score-board">
              ...
              <b className="muted-team">{secondTeam}</b>
            </div>
          </div>
          <div className="analysis-body">
            <h3>Diễn biến chính</h3>
            {detail.events.length === 0 ? (
              <p className="match-muted">Chưa có diễn biến được ghi nhận.</p>
            ) : (
              <div className="match-timeline">
                ...
                <div className="timeline-list">
                  {detail.events.map((event) => {
                    ...
                    const isSub = event.eventType.includes("sub");
                    ...
                    const icon = isGoal
                      ...
                      : isCard
                        ...
                        : isSub
                          ? "🔁"
                          : "⏱️";
                    const player = event.playerUserId
                      ? (event.playerDisplayName ??
                        `Cầu thủ ${event.playerUserId.slice(0, 8)}`)
                      : null;
                    const secondary = event.secondaryUserId
                      ? (event.secondaryDisplayName ??
                        `Cầu thủ ${event.secondaryUserId.slice(0, 8)}`)
                      : null;
                    const identity = [player, secondary]
                      .filter(Boolean)
                      .join(" → ");
                    return (
                      <div
            ...
            {mvp && (
              <div className="motm">
                <div className="initial-avatar dark-avatar">MVP</div>
                <div>
                  <span>CẦU THỦ XUẤT SẮC</span>
                  <b>{mvp.displayName ?? "Cầu thủ"}</b>
                  <small>
                    {mvp.goals} bàn • {mvp.assists} kiến tạo •{" "}
                    {mvp.rating ?? "—"} điểm
                  </small>
                </div>
                <Trophy />
            ...
            {metricRows(detail.teamMetrics).length === 0 ? (
              <p className="match-muted match-metrics-empty">
                Chưa ghi nhận chỉ số hai đội.
              </p>
            ) : (
              <div className="team-stats">
                {metricRows(detail.teamMetrics).map(([label, metric]) => (
        ...
        <article className="card match-attendance-card">
          <div className="section-head">
            <div>
              <span>THAM GIA</span>
          ...
          {detail.attendance.length === 0 ? (
            ...
            (() => {
              ...
              return (
                <div className="rsvp-split-section">
                  <div className="rsvp-group-title">
                    <span>Đội hình chính (7 người)</span>
                    <span className="starting-pill">
                      {Math.min(7, available.length)}/7
                    </span>
                  </div>
                  {available.slice(0, 7).map((attendance) => (
                    <div
                      className="match-attendance-row"
                      key={attendance.userId}
                    >
                      <b>{attendance.displayName ?? "Thành viên"}</b>
                      <span className="starting-pill">Đội chính</span>
                    </div>
                  ))}
                  {available.length > 7 && (
                    <>
                      <div
                        className="rsvp-group-title"
                        style={{ marginTop: 8 }}
                      >
                        <span>Dự bị sẵn sàng</span>
                        <span className="bench-pill">
                          {available.length - 7} cầu thủ
                        </span>
                      </div>
                      {available.slice(7).map((attendance) => (
                        <div
                          className="match-attendance-row"
                          key={attendance.userId}
                        >
                  ...
                  {others.length > 0 && (
                    <>
                      <div
                        ...
                        style={{ marginTop: 8 }}
                      >
                        <span>Vắng mặt & Đang chờ</span>
                        <span style={{ fontSize: 12, color: "var(--muted)" }}>
                          {others.length}
                        </span>
                      </div>
                      {others.map((attendance) => (
                        <div
                          className="match-attendance-row"
                          key={attendance.userId}
                        >
                          <b style={{ color: "var(--muted)" }}>
                            {attendance.displayName ?? "Thành viên"}
                          </b>
      ...
      {canManage && match.status === "completed" && (
        <MatchAnalysisEditor
          ...
          teamMetrics={detail.teamMetrics}
          candidates={detail.analysisCandidates}
        />
      )}
      {canManage && match.status === "scheduled" && (
        <section className="match-admin-grid">
          ...
          >
            ...
            <button className="primary-button" disabled={state.pending}>
              <Save />
              Lưu thay đổi
            </button>
          </form>
          <div className="view-stack">
            ...
            <section className="card match-invite-card">
              <div className="section-head">
                <div>
                  <span>ĐIỂM DANH</span>
              ...
              <div className="match-invite-list">
                {detail.inviteCandidates.map((candidate) => (
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSelected((current) => {
                          ...
                          if (event.target.checked) next.add(candidate.userId);
                          else next.delete(candidate.userId);
                          return next;
                        })
                      }
                    />
                    <span>{candidate.displayName ?? "Thành viên"}</span>
                    {candidate.invited && <small>Đã mời</small>}
                  </label>
                ))}
              </div>
              <button
            ...
            <div className="form-two">
              <label>
                ...
                <input
                  ...
                  type="number"
                  name="teamScore"
                  defaultValue={match.teamScore ?? 0}
                />
              </label>
              <label>
                Tỉ số đối thủ ({match.opponent})
                <input
                  required
                  min="0"
                  max="32767"
