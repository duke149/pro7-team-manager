...
export async function renderTacticsMatchPage(arguments_: Arguments) {
  ...
  const canEdit = canManage && result.detail.match.status === "scheduled";
  if (!canEdit && result.detail.tactics.length === 0) return <div className="view-stack tactics-view" data-state="empty"><section className="card tactics-state"><h2>Chưa có chiến thuật đã áp dụng</h2><p>Quản lý đội chưa công bố đội hình cho trận này.</p></section></div>;
  return <TacticsBoard slug={context.team.slug} teamName={context.team.name} detail={result.detail} canManage={canEdit} />;
}
