...
async function main() {
  ...
  await desktopPage.fill('#login-identifier', process.env.PRO7_TEST_USER || "admin");
  await desktopPage.fill('#login-password', process.env.PRO7_TEST_PASSWORD || "");
  await Promise.all([
  ...
  await mobilePage.fill('#login-identifier', process.env.PRO7_TEST_USER || "admin");
  await mobilePage.fill('#login-password', process.env.PRO7_TEST_PASSWORD || "");
  await Promise.all([
