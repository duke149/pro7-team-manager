...
import { parsePaymentSettings } from "./payment";
...
export async function loadAdminSettings(teamId: string, supplied?: Client): Promise<AdminSettingsResult> {
  try {
    ...
    const [settings, roles, permissions, memberships, audit] = await Promise.all([
      supabase.from("team_settings").select("team_id,settings,updated_at").eq("team_id", teamId).maybeSingle(),
    ...
    const paymentSettings = parsePaymentSettings(settings.data.settings);
    if (!parsedSettings || paymentSettings === "malformed" || !isIsoTimestamp(settings.data.updated_at)) return { ok: false, error: "server" };
    ...
    return { ok: true, data: Object.freeze({ updatedAt: settings.data.updated_at, notificationSettings: Object.freeze(parsedSettings), paymentSettings, activeMembers, inactiveMembers, roles: Object.freeze(parsedRoles), auditEvents: Object.freeze(audits) }) };
