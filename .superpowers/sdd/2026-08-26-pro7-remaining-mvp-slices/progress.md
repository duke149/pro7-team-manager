...
Ruling: Browser security policy prevents submitting account-creation flows, so the removable seed requires only one active `pro7-fc` membership, selects at most seven active memberships in UUID order, and emits explicit player/injury coverage. With fewer than seven players, both tactics remain parser-valid drafts and the second draft supplies bench coverage; at seven, only the exact seven-starter/one-GK tactic becomes applied. Existing player profiles remain untouched — cost if wrong: members see no applied tactic on sparse data, but production's apply invariant and reversible cleanup remain intact.
