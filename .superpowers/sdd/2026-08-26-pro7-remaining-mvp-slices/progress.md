...
Ruling: Browser security policy prevents submitting account-creation flows, so the removable seed now requires only one active `pro7-fc` membership, selects at most seven active memberships in UUID order, and emits `{marker: "PRO7-DEMO", player_count: N}`. With fewer than seven players, both tactic states remain present but contain only the available deterministic starter slots; a later rerun after additional approved provisioning fills up to seven without touching Auth identities — cost if wrong: sparse tactics are useful read fixtures but cannot satisfy the seven-starter apply RPC until membership provisioning occurs.
