...
- `npm run lint` retains seven baseline `app/pro7-app.tsx` accessibility errors plus one `<img>` warning (eight total); direct `tsc --noEmit` retains baseline Cloudflare Worker/factory/test type failures. Treat either as separate cleanup work, not as proof of this slice regressing.
